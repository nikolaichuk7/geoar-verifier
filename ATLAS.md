...
|---|---|---|---|---|
| AWS Nitro Enclaves attestation document | AWS Nitro Root G1 chain (in the COSE payload) | region only as a hostname label in the certificate CNs (leaf, instance, zonal and regional CAs); the zone is never named; L/ST/C are the issuer's own address | Endorsement | measured, 12 documents, us-east-2 / eu-west-1 (May 2026, private corpus) + 3 nonce-bound documents, us-east-2a (11 Sep 2026, public) → `aws-nitro/` |
...
| AWS NitroTPM (EC2 virtual TPM) | no certificate: the EK public key is returned by the EC2 API `GetInstanceTpmEkPub` to the account owner; the guest's EK matches it | none in any TPM artifact; the place lives in the API context and in the instance identity document | Endorsement without a certificate | measured, 2 instances, us-east-2a (11 Sep 2026) → `aws-nitro/` |
