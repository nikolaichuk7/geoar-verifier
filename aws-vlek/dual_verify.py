...
def main(root):
    summary = []
    chains = {}
    for kind in ("vlek", "vcek"):
        try: chains[kind] = x509.load_pem_x509_certificates(get(f"https://kdsintf.amd.com/{kind}/v1/Milan/cert_chain"))
        except Exception as e: chains[kind] = None
    for d in sorted(glob.glob(os.path.join(root, "*", "2026*"))):
        ...
        for p in sorted(glob.glob(os.path.join(d, "report-*.bin"))):
            ...
            r = {"signing_key": f["signing_key"], "chip_id_zero": f["chip_id_zero"], "chip_id_prefix": f["chip_id"][:16], "report_id": f["report_id"][:16], "report_data_is_nonce": f["report_data"] == N.hex()}
            ...
            if verified_by is None and not f["chip_id_zero"]:
                ...
                try:
                    ...
                    if sig_ok(rep, vcek): verified_by = "kds:VCEK"; leaf = vcek
                except Exception as e: r["kds_error"] = repr(e)[:80]
            r["verified_by"] = verified_by
            if verified_by:
                kind = "vlek" if f["signing_key"] == "VLEK" else "vcek"; ch = chains.get(kind)
